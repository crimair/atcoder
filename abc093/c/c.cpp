...
int main() {
  ...
  int dx[6] = {1, 0, 1, 2, 0, 0};
  int dy[6] = {1, 1, 0, 0, 2, 0};
  int dz[6] = {0, 1, 1, 0, 0, 2};
  openlist.push_back(p3(iv[0], iv[1], iv[2]));
  while (true) {
    tmplist.clear();

    while (!openlist.empty()) {
      auto cv = openlist.front();
      // cout << cv << endl;
      if (cv.x == cv.y && cv.y == cv.z) {
        cout << cost << endl;
        return 0;
      }

      bool check = true;
      for (auto dv : closelist) {
        if (cv == dv) {
          check = false;
          break;
        }
      ...
      if (check) {
        closelist.push_back(p3(cv.x, cv.y, cv.z));
        FOR(i, 0, 6) {
          tmplist.push_back(p3(cv.x + dx[i], cv.y + dy[i], cv.z + dz[i]));
        }
      }
    ...
    cost++;
    for (auto tv : tmplist) {
      openlist.push_back(p3(tv.x, tv.y, tv.z));
    }
  }
